...
кодовых репозиториях `cybercity-*` (хаб `cybercity` — обложка со своим
составом, без кода и `AGENTS.md`); каждый `AGENTS.md` ссылается сюда. Если
локальное правило репозитория противоречит этому файлу — побеждает этот файл
(или заводится новый ADR).
...
2. **`cybercity/adr/`** — все архитектурные решения проекта (и сквозные, и
   локальные); в репозиториях `docs/adr/` нет (ADR-0005). Статус `superseded`
   не имеет силы; `amended` — см. ссылку.
...
ADR в `cybercity/adr/`; старые ADR помечаем `superseded`, **не удаляем**.
...
- ADR — `NNNN-kebab-case.md` (`0001-two-graph-architecture.md`), нумерация
  сквозная по всему проекту (ADR живут только в хабе `cybercity/adr/`, см.
  ADR-0005), с ведущими нулями.
...
- Минимальное содержание `AGENTS.md`: ссылка на хаб `COMPOSITION.md` и
  ...
  языка.
- Коммиты, пуши, PR — делает агент; человек их только апрувит (не пишет сам).
  Агент коммитит/пушит только после явного запроса человека; на default-ветке
  сначала заводит отдельную ветку.

## ADR — формат

- Отдельный файл `cybercity/adr/NNNN-kebab.md` (все ADR — в хабе, нумерация
  сквозная по всему проекту; см. ADR-0005).
...
- `## Scope` — чему относится решение: `Сквозное` либо репозиторий(и)
  (`engine`, `data`, …). Поле нужно, потому что локальные ADR репозиториев
  живут в общем каталоге и должны быть различимы.
- `Status`: `Accepted` | `Superseded` (со ссылкой на заменивший) |
  `Amended` (со ссылкой).
- `cybercity/adr/README.md` — индекс: номер, scope, заголовок, статус, ссылка.
...
Таблица состава проекта (семь репозиториев, включая хаб-обложку), контракты и
доверительная граница описаны **только** в `cybercity/COMPOSITION.md`. Ни один
репозиторий не копирует её таблицей; все ссылаются на хаб. При расхождении
канон — в хабе.
