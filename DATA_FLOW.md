...
3. **Валидация** — `engine` проверяет событие против схемы и текущего
   `WorldState`; для эксплойта vuln — также `requires`-предусловие из
   [ADR-0006](adr/0006-vulnerability-declarative-overlay-realism.md) (V2 валиден,
   только если V1 уже эксплуатируется — иначе помечается). Неподписанные/невалидные
   — отбрасываются или помечаются `status: failed/suppressed`.
...
5. **Наблюдение исхода (propagation)** — движок **не выводит** исход из графа.
   Коллектор наблюдает состояние каждого сервиса out-of-band; при смене состояния
   (compromised и т.п.) шлёт подписанное событие — движок записывает. Атака
   расползается потому, что коллектор видит, как игрок реально движется по рёбрам
   достижимости топологического графа, и сообщает каждое падение. Движок —
...
sequenceDiagram
    ...
    E->>E: записать команду (исход не выводится — ждём коллектора)
...
симулятор; ADR-0008: пропагация наблюдается, не вычисляется). Игрок бьёт по
реальному сокету `clite` (range); коллектор видит это снаружи и сообщает. UI
отражает смену состояния live.
