...
| [`ARCHITECTURE.md`](ARCHITECTURE.md) | Как соединено? | Системный контекст, два графа, hybrid execution, слои развёртывания, roadmap. |
...
| 🧩 | [cybercity-clite](https://github.com/TheCipherKeeper/cybercity-clite) | Stub-образ `cc-lite` для `runtime_kind: lite` (Rust) |
...
3. **Как соединено** — [`ARCHITECTURE.md`](ARCHITECTURE.md): системный контекст, два графа, слои, roadmap.
4. **Как работаем** — [`CONVENTIONS.md`](CONVENTIONS.md): иерархия доков, язык, скелет репо, форматы.
5. **Почему так** — [`adr/`](adr/): все архитектурные решения (сквозные и локальные).
